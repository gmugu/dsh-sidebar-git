...
function defaultExpandedFiles(files) {
  ...
  if (only !== void 0) return expandable(only) ? /* @__PURE__ */ new Set([0]) : /* @__PURE__ */ new Set();
  const expanded = /* @__PURE__ */ new Set();
  files.forEach((file, index) => {
    const path = displayPath(file.newPath === "/dev/null" ? file.oldPath : file.newPath);
    if (expandable(file) && !TEST_PATH.test(path) && !DOC_PATH.test(path) && !GENERATED_PATH.test(path) && SOURCE_PATH.test(path)) {
      expanded.add(index);
    }
  });
  return expanded;
